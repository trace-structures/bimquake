...
class Building(IfcObjectSet):
    ...
    def get_building_excel(self, file_path = 'building.xlsx'):
        ...
        with pd.ExcelWriter(writer) as writer:
            ...
            for idx, (sheet_name, df) in enumerate(props.items()):
                print(f"Writing sheet {idx}: {sheet_name} with shape {df.shape}")
                df.to_excel(writer, sheet_name='Floor{}'.format(idx+1), index=False)
